...
- **True FP16 depth** — *built + measured on the A10G*: `export_onnx.py --fp16` emits a uniformly-fp16 ONNX (all weights + forced fp16 I/O + retargeted internal `Cast(to=fp32)` nodes) and `compile_trt.py --fp16` builds a `STRONGLY_TYPED` TensorRT 11 engine that runs genuine FP16; `DepthEstimator` is dtype-aware (reads each binding's dtype, casts host↔device) so it runs either engine. Measured **TF32 14.2 ms → FP16 6.3 ms (2.26×)** at **corr 0.99996** fidelity vs TF32 (`scripts/bench_depth.py`), which — with the CUDA TSDF kernel — takes the live pipeline to **93.2 FPS**. Network-flag logic unit-tested (`tests/test_compile_trt_flags.py`); `scripts/inspect_onnx.py` dumps graph dtypes for debugging.
