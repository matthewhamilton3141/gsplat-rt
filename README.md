...
- **CUDA TSDF kernel** — *kernel written, CPU-validated*: a custom one-thread-per-voxel integrate kernel (`kernels/tsdf_integrate.cu`) with a torch binding + numpy fallback (`src/mapping/tsdf_cuda.py`) replaces the numpy 64³ integrator (the one over-budget stage, 13 ms on the A10G host). Its arithmetic is verified against the production numpy path bit-for-bit via a CPU oracle (`tests/test_tsdf_cuda.py`); GPU parity + the `scripts/bench_tsdf.py` speed-up land on the next A10G build (`python setup.py build_ext --inplace`). Next: wire the kernel into `TSDFVolume`'s live path (persistent device volume, lazy host sync for mesh/occupancy), targeting sub-millisecond.
